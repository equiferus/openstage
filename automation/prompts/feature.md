...
Read the issue, every comment, and open-PR context supplied below. Treat the Product review's implementation scope and acceptance criteria as authoritative. Load and follow the repository `github-issue-delivery` skill. Inspect architecture and tests before editing. If an existing issue PR is supplied, this is a PM-requested revision: check out its existing `issue/<number>-*` branch, address the latest delivery feedback, push an updated commit to that branch, and update the existing PR. Never create a duplicate PR.
